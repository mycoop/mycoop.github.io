...
$(function() {
    ...
    d3.json('/force-tree/les-mis.json', function(data) {
...
//        }
        ...
        _.each(data, function(item){
            var obj = item;
            var dependsOnBy = [];
            _.each(data, function(depend){
                if(_.contains(depend.depends, item.name)){
                    dependsOnBy.push(depend.name);
                }
            });
            item.dependedOnBy = dependsOnBy;
